...
export function Hero() {
  ...
  useEffect(() => {
    ...
    type Piece = {
      ...
      type: "rect" | "circle" | "ribbon"
      length?: number; wave?: number; waveSpeed?: number; waveAmp?: number
    }

    const colors = ["#f97316", "#7c3aed", "#ec4899", "#facc15", "#34d399", "#60a5fa", "#f43f5e", "#a855f7", "#fb923c", "#ffffff"]
    const pieces: Piece[] = []
    ...
    for (let i = 0; i < 18; i++) {
      pieces.push({
        ...
        waveSpeed: Math.random() * 0.07 + 0.03,
        waveAmp: Math.random() * 9 + 5,
      })
    }
    ...
    const draw = () => {
      ...
      pieces.forEach((p) => {
        ...
        if (p.type === "rect") {
          ...
          ctx.fillRect(-p.size / 2, -p.size / 3, p.size, p.size / 1.8)
          ...
          const amp = p.waveAmp ?? 6
          for (let t = 0; t <= len; t += 2) {
            const wx = t
            const wy = Math.sin((t / len) * Math.PI * 2 + (p.wave ?? 0)) * amp
            if (t === 0) ctx.moveTo(wx, wy)
            else ctx.lineTo(wx, wy)
          }
          ctx.stroke()
          if (p.wave !== undefined && p.waveSpeed !== undefined) p.wave += p.waveSpeed
        }
        ctx.restore()
  ...
  return (
    <section style={{
      ...
      <div style={{
        ...
        padding: "clamp(7px,1.3vh,12px) clamp(14px,2vw,32px)",
        flexWrap: "nowrap", gap: 10, zIndex: 10,
      }}>
        {/* СЛЕВА */}
        <div style={{ display: "flex", alignItems: "center", gap: "clamp(8px,1.5vw,24px)", flexWrap: "wrap", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
            <span style={{ width: 38, height: 38, borderRadius: "50%", background: "linear-gradient(135deg,#7c3aed,#a855f7)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, color: "#fff", flexShrink: 0 }}>🕐</span>
            <div>
              <div style={{ fontWeight: 800, fontSize: "clamp(11px,1.1vw,15px)", color: "#1e1b4b", fontFamily: "'Montserrat',sans-serif" }}>Работаем 24/7</div>
              <div style={{ fontSize: "clamp(10px,0.85vw,12px)", color: "#6b7280", fontFamily: "'Montserrat',sans-serif" }}>без выходных</div>
            </div>
          </div>
          <div style={{ width: 2, height: 34, background: "rgba(124,58,237,0.25)", borderRadius: 2, flexShrink: 0 }} />
          <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
            <span style={{ width: 38, height: 38, borderRadius: "50%", background: "linear-gradient(135deg,#7c3aed,#a855f7)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, color: "#fff", flexShrink: 0 }}>📍</span>
            <div>
              <div style={{ fontWeight: 800, fontSize: "clamp(11px,1.1vw,15px)", color: "#1e1b4b", fontFamily: "'Montserrat',sans-serif" }}>ул. Героя Яцкова, 19к3</div>
              <div style={{ fontSize: "clamp(10px,0.85vw,12px)", color: "#6b7280", fontFamily: "'Montserrat',sans-serif" }}>г. Краснодар</div>
            </div>
          </div>
        </div>

        {/* СПРАВА: соцсети */}
        <div style={{ display: "flex", alignItems: "center", gap: "clamp(4px,0.6vw,8px)", flexWrap: "wrap" }}>
          {[
            { label: "WhatsApp", href: "https://wa.me/79885973303", bg: "#25D366",
              icon: <svg width="17" height="17" viewBox="0 0 32 32" fill="white"><path d="M16 2C8.28 2 2 8.28 2 16c0 2.46.66 4.76 1.8 6.76L2 30l7.44-1.76A13.9 13.9 0 0016 30c7.72 0 14-6.28 14-14S23.72 2 16 2zm0 25.4c-2.18 0-4.22-.6-5.96-1.64l-.42-.26-4.42 1.04 1.06-4.3-.28-.44A11.38 11.38 0 014.6 16C4.6 9.7 9.7 4.6 16 4.6S27.4 9.7 27.4 16 22.3 27.4 16 27.4zm6.26-8.54c-.34-.17-2.02-.998-2.334-1.112-.312-.114-.54-.17-.766.17-.228.34-.882 1.112-1.082 1.34-.198.228-.396.256-.73.086-.336-.17-1.416-.522-2.698-1.664-.998-.888-1.672-1.986-1.868-2.32-.196-.336-.02-.518.148-.686.152-.152.336-.396.504-.594.17-.198.226-.34.338-.566.114-.228.056-.428-.028-.596-.086-.17-.766-1.842-1.048-2.522-.276-.66-.558-.57-.766-.582-.198-.01-.426-.012-.654-.012-.228 0-.596.086-.908.426-.312.34-1.192 1.164-1.192 2.838s1.22 3.294 1.39 3.522c.17.228 2.4 3.666 5.814 5.138.812.35 1.446.56 1.94.716.814.26 1.556.224 2.142.136.654-.098 2.014-.822 2.298-1.616.284-.794.284-1.474.198-1.616-.084-.14-.312-.226-.646-.396z"/></svg> },
            { label: "Telegram", href: "https://t.me/sharovik_krd", bg: "#229ED9",
              icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="white"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg> },
            { label: "ВКонтакте", href: "https://vk.com/sharovik_krd", bg: "#0077FF",
              icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="white"><path d="M15.07 2H8.93C3.33 2 2 3.33 2 8.93v6.14C2 20.67 3.33 22 8.93 22h6.14C20.67 22 22 20.67 22 15.07V8.93C22 3.33 20.67 2 15.07 2zm3.08 13.5h-1.85c-.7 0-.91-.56-2.16-1.81-1.09-1.05-1.57-.93-1.57.22v1.65c0 .39-.13.62-1.18.62-1.74 0-3.67-1.05-5.03-3.01C4.42 10.5 4 8.5 4 8.5s0-.23.25-.23h1.85c.68 0 .94.31 1.19 1.04.65 1.9 1.74 3.57 2.19 3.57.17 0 .25-.08.25-.5V10.1c-.06-1.06-.62-1.15-.62-1.53 0-.21.17-.42.45-.42h2.9c.38 0 .51.21.51.66v3.12c0 .38.17.51.28.51.17 0 .31-.13.62-.44 1.07-1.2 1.83-3.06 1.83-3.06.1-.23.3-.44.68-.44h1.85c.56 0 .68.29.56.67-.23.97-2.42 4.14-2.42 4.14-.19.31-.26.45 0 .79.19.26.8.79 1.21 1.27.74.84 1.31 1.55 1.46 2.04.17.49-.08.74-.57.74z"/></svg> },
            { label: "Instagram", href: "https://instagram.com/sharovik_krd", bg: "linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)",
              icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="white"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg> },
            { label: "Max", href: "https://vk.com/sharovik_krd", bg: "#1e3a5f",
              icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="white"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z"/></svg> },
          ].map((s) => (
            <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" title={s.label}
              style={{
                display: "flex", alignItems: "center", gap: 6,
                background: s.bg, borderRadius: 999,
                padding: "clamp(6px,0.85vh,10px) clamp(9px,1.1vw,16px)",
                fontWeight: 700, fontSize: "clamp(11px,0.9vw,13px)",
              ...
              onMouseEnter={e => {
                ...
                ;(e.currentTarget as HTMLElement).style.boxShadow = "0 5px 16px rgba(0,0,0,0.28)"
              }}
              onMouseLeave={e => {
                (e.currentTarget as HTMLElement).style.transform = "translateY(0) scale(1)"
